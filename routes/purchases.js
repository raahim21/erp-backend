...
router.post(
  ...
  requireRole("admin", "manager"),
  [
    body("type").isIn(["Vendor", "Internal", "Transfer"]).withMessage("Invalid purchase type"),
    body("vendorId").if(body("type").equals("Vendor")).isMongoId().withMessage("Invalid vendor ID"),
    body("department").if(body("type").equals("Internal")).isString().trim().notEmpty().withMessage("Department is required and cannot be empty for Internal type"),
    body("fromLocation").if(body("type").equals("Transfer")).isMongoId().withMessage("Invalid from location ID"),
    body("toLocation").isMongoId().withMessage("Invalid to location ID"), // Required for all
    body("productId").isMongoId().withMessage("Invalid product ID"),
    body("quantity").isInt({ min: 1 }).withMessage("Quantity must be at least 1"),
    body("totalPrice").isFloat({ min: 0 }).withMessage("Total price must be at least 0"),
    body("status").optional().isIn(["Pending", "Completed", "Cancelled"]).withMessage("Invalid status"),
    body("poNumber").optional().isString().trim().withMessage("Invalid PO number"),
...
// GET /api/purchases
router.get("/", auth, purchasesController.getPurchases);
...
router.put(
  ...
  requireRole("admin", "manager"),
  [
    body("type").optional().isIn(["Vendor", "Internal", "Transfer"]).withMessage("Invalid purchase type"),
    body("vendorId").optional().if(body("type").equals("Vendor")).isMongoId().withMessage("Invalid vendor ID"),
    body("department").optional().if(body("type").equals("Internal")).isString().trim().notEmpty().withMessage("Department cannot be empty for Internal type"),
    body("fromLocation").optional().if(body("type").equals("Transfer")).isMongoId().withMessage("Invalid from location ID"),
    body("toLocation").optional().isMongoId().withMessage("Invalid to location ID"),
    body("productId").optional().isMongoId().withMessage("Invalid product ID"),
    body("quantity").optional().isInt({ min: 1 }).withMessage("Quantity must be at least 1"),
    body("totalPrice").optional().isFloat({ min: 0 }).withMessage("Total price must be at least 0"),
    body("status").optional().isIn(["Pending", "Completed", "Cancelled"]).withMessage("Invalid status"),
    body("notes").optional().isString().trim().withMessage("Invalid notes"),
  ],
  handleValidationErrors,
